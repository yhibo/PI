...
for patient in patients:
    ...
    print("Segmento")
    ####### SEGMENTACION
    seg_folder = os.path.join(output_folder,'images','their_seg')
    ...
    nib.save(aha_img, os.path.join(output_folder,'images','their_seg',f"{patient}_aha.nii.gz"))
    print("Shape de aha: ", np.shape(aha_img))

    dfield_folder = os.path.join(output_folder,'dfield','their_seg')
    np.save(os.path.join(dfield_folder, f"{patient}_dfield.npy"), dfield)
    print("Shape de dfield: ", np.shape(dfield))

    strain_folder = os.path.join(output_folder,'strain','their_seg')
