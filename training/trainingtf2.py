...
parser.add_argument('--loss_weights', type=float, nargs='+',
                    default=[0.01, 0.5, 0.1, 0], help='Loss weights')
...
def loss_new(y_true, y_pred):
    ...
    # Consistency loss
    L_c = K.mean(K.abs(K.mean(K.cast(M_0 == 0, 'float32'),
                              axis=(0, 1, 2, 4)) -
                       K.mean(K.cast(M_0_pred0 > 0.5, 'float32'),
                              axis=(0, 1, 2, 4))))
    L_c += K.mean(K.abs(K.mean(K.cast(M_0 == 1, 'float32'),
                               axis=(0, 1, 2, 4)) -
                        K.mean(K.cast(M_0_pred1 > 0.5, 'float32'),
                               axis=(0, 1, 2, 4))))
    L_c += K.mean(K.abs(K.mean(K.cast(M_0 == 2, 'float32'),
                               axis=(0, 1, 2, 4)) -
                        K.mean(K.cast(M_0_pred2 > 0.5, 'float32'),
                               axis=(0, 1, 2, 4))))
    L_c += K.mean(K.abs(K.mean(K.cast(M_0 == 3, 'float32'),
                               axis=(0, 1, 2, 4)) -
                        K.mean(K.cast(M_0_pred3 > 0.5, 'float32'),
                               axis=(0, 1, 2, 4))))
    L_c /= 4

    return (lambda_i * L_i + lambda_a * L_a +
            lambda_s * L_s + lambda_a + lambda_c * L_c)
...
class Augment(tf.keras.layers.Layer):
    def __init__(self, seed=42):
        ...
        self.augment_batch = tf.keras.Sequential([
            ...
            tf.keras.layers.RandomTranslation(0.3, 0.3, seed=None),
            tf.keras.layers.RandomZoom(0.3, seed=None)
            ])
    ...
    def call(self, inputs, labels):
        ...
        ## adjust gamma
        gamma = tf.random.uniform(shape=[1,], minval=0.8, maxval=1.2)
        gain = tf.random.uniform(shape=[1,], minval=0.8, maxval=1)
        augmented = tf.image.adjust_gamma(augmented, gamma=gamma, gain=gain)

        inputs['input_1'] = tf.expand_dims(tf.stack(
            tf.unstack(augmented[..., :nz], axis=-1), axis=-1), axis=-1)
        inputs['input_2'] = tf.expand_dims(tf.stack(
            tf.unstack(augmented[..., nz:2*nz], axis=-1), axis=-1), axis=-1)

        labels = tf.stack([
            tf.expand_dims(tf.stack(tf.unstack(
                augmented[..., 2*nz:3*nz], axis=-1), axis=-1), axis=-1),
            tf.expand_dims(tf.stack(tf.unstack(
                augmented[..., 3*nz:4*nz], axis=-1), axis=-1), axis=-1),
            tf.expand_dims(tf.stack(tf.unstack(
                augmented[..., 4*nz:5*nz], axis=-1), axis=-1), axis=-1),
            tf.expand_dims(tf.stack(tf.unstack(
                augmented[..., 5*nz:6*nz], axis=-1), axis=-1), axis=-1),
            tf.expand_dims(tf.stack(tf.unstack(
                augmented[..., 6*nz:7*nz], axis=-1), axis=-1), axis=-1),
            tf.expand_dims(tf.stack(tf.unstack(
                augmented[..., 7*nz:8*nz], axis=-1), axis=-1), axis=-1),
            tf.expand_dims(tf.stack(tf.unstack(
                augmented[..., 8*nz:9*nz], axis=-1), axis=-1), axis=-1)
            ], axis=1)
...
model_name += ('_loss_weights_' + str(lambda_i) +
               '_' + str(lambda_a) + '_' + str(lambda_s))
if len(args.loss_weights) == 4:
