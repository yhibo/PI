...
def dense_image_warp3d(image, flow, indexing='ij', name=None):
    ...
    with tf.name_scope(name or "dense_image_warp"):
        ...
        query_points_flattened = tf.reshape(query_points_on_grid, [nx * ny * nz, 3])

        interpolated = interpolate_bilinear(image, query_points_flattened)
        interpolated = tf.reshape(interpolated, [nx, ny, nz, channels])
    ...
    with tf.name_scope(name or "interpolate_bilinear"):
        ...
        with tf.control_dependencies([
                tf.debugging.assert_equal(
                    query_shape[1],
                    3,
                    message="Query points must be size 3 in dim 2.")
        ]):
        ...
        flattened_grid = tf.reshape(
            grid, [height * width * depth, channels])
        ...
        def gather(y_coords, x_coords, z_coords, name):
            ...
            return tf.reshape(gathered_values,  [num_queries, channels])
