...
#     lambda_s = np.array(0.3, dtype= np.float32)
...
def criterion_netME(y_true, y_pred):
    ...
    lambda_i = np.array(0.01, dtype= np.float32)
    lambda_a = np.array(0.5, dtype= np.float32)
    lambda_s = np.array(0.5, dtype= np.float32)
...
netME.save_weights("netME_weights_new_EDES_12_300_reg_dice_res_001_05_05.h5")
